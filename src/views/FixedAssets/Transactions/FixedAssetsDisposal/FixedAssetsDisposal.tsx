                ...
                try {
                    ...
                    setRows(prev => prev.map(row => {
                        if (row.selectedItemId) {
                            const itemStockMoves = allStockMoves.filter((move: any) => move.stock_id === row.selectedItemId && move.loc_code === location);
                                        ...
                                        onChange={async (e) => {
                                            ...
                                            if (selectedItem) {
                                                ...
                                                try {
                                                    ...
                                                    // Set unit cost from purchase_cost
                                                    if (itemData && itemData.purchase_cost !== undefined) {
                                                        handleChange(row.id, "unitCost", String(itemData.purchase_cost));
                                                    }
                                                    // Fetch QOH from stock_moves if from location is selected
                                                    if (location) {
                                                        const allStockMoves = await getStockMoves();
                                                        const itemStockMoves = allStockMoves.filter((move: any) => move.stock_id === selectedItem.stock_id && move.loc_code === location);
                                                        const qoh = itemStockMoves.reduce((sum: number, move: any) => sum + (move.qty || 0), 0);
