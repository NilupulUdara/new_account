...
export default function FixedAssetsPurchase() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [suppliersData, locationsData, dimensionsData, itemsData, itemUnitsData, paymentTypesData, categoriesData] = await Promise.all([
          getSuppliers(),
  ...
  const breadcrumbItems = [
    ...
    { title: "Fixed Asset Purchase Invoice Entry" },
  ];
  ...
  return (
    <Stack spacing={2}>
      ...
      >
        <Box>
          <PageTitle title="Fixed Asset Purchase Invoice Entry" />
