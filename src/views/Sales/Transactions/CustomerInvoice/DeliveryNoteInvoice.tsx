...
import { createDebtorTransDetail, updateDebtorTransDetail } from "../../../../api/DebtorTrans/DebtorTransDetailsApi";
...
export default function DeliveyNoteInvoice() {
  ...
  const updateDetailMutation = useMutation({
    mutationFn: (variables: { id: string | number; data: any }) =>
      updateDebtorTransDetail(variables.id, variables.data),
  });
  ...
  useEffect(() => {
    ...
    if (stateReference && debtorTrans.length > 0 && customers.length > 0) {
      ...
      if (delivery) {
        ...
        const deliveryDetails = debtorTransDetails.filter((dd: any) => dd.debtor_trans_no === delivery.trans_no && dd.debtor_trans_type === 13);
        ...
        if (deliveryDetails.length > 0) {
          const newRows = deliveryDetails.map((detail: any, index: number) => {
            ...
            return {
              ...
              delivered: qty,
              units: unitName,
              invoiced: parseFloat(detail.qty_done || 0), // Show already invoiced quantity from delivery details
              thisInvoice: 1,
              price: price,
              taxType: taxTypeData?.name,
              discount: parseFloat(detail.discount_percent || 0),
              total: total,
              src_id: detail.id, // Reference the delivery detail's primary key
              unitTax: unitTax,
  ...
  const handleProcessInvoice = async () => {
    ...
    try {
      ...
      // Update delivery details to reflect invoiced quantities
      const deliveryDetails = debtorTransDetails.filter((dd: any) => dd.debtor_trans_no === delivery.trans_no && dd.debtor_trans_type === 13);
      const updatePromises = filteredRows.map(r => {
        const deliveryDetail = deliveryDetails.find((dd: any) => dd.id === r.src_id);
        if (deliveryDetail) {
          const currentQtyDone = parseFloat(deliveryDetail.qty_done || 0);
          const newQtyDone = currentQtyDone + r.thisInvoice;
          return updateDetailMutation.mutateAsync({
            id: deliveryDetail.id,
            data: { qty_done: newQtyDone },
          });
        }
      ...
      // Create payment transaction with trans_type = 12
      const paymentTransNo = newTransNo + 1;
      const paymentData = {
        ...
        tpe: delivery.tpe,
        tran_date: date,
        ov_amount: invoiceTotal, // Negative amount for payment
        alloc: invoiceTotal,
        due_date: dueDate || date,
        payment_terms: paymentTerm,
        shipping_company: shippingCompany,
      ...
      await createTransMutation.mutateAsync(paymentData);
