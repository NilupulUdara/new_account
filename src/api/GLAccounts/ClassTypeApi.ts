...
const API_URL = "http://127.0.0.1:8000/api/class-types";

export const createClassType = async (classTypeData: any) => {
  try {
    const response = await axios.post(API_URL, classTypeData);
...
export const updateClassType = async (id: string | number, classTypeData: any) => {
  try {
    const response = await axios.put(`${API_URL}/${id}`, classTypeData);
    return response.data;
  } catch (error: any) {
    console.error(error.response?.data || error);
    throw error.response?.data || error;
